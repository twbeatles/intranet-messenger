...
// ============================================================================
// 이모지 반응 (v4.0에서 toggleReaction으로 이전)
// ============================================================================
// 리액션 기능은 파일 하단의 v4.0 리액션 시스템으로 이전됨
...
function setupEventListeners() {
    ...
    if ($('userAvatar')) $('userAvatar').onclick = openProfileModal;
    if ($('userInfoClick')) $('userInfoClick').onclick = openProfileModal;
    if ($('closeProfileModal')) $('closeProfileModal').onclick = closeProfileModal;
    if ($('saveProfileBtn')) $('saveProfileBtn').onclick = saveProfile;
    if ($('changeProfileImageBtn')) $('changeProfileImageBtn').onclick = () => $('profileImageInput').click();
    if ($('profileImageInput')) $('profileImageInput').onchange = handleProfileImageUpload;
    if ($('deleteProfileImageBtn')) $('deleteProfileImageBtn').onclick = deleteProfileImage;

    // 멤버 모달 이벤트
    if ($('closeMembersModal')) $('closeMembersModal').onclick = () => $('membersModal').classList.remove('active');
    if ($('closeMembersBtn')) $('closeMembersBtn').onclick = () => $('membersModal').classList.remove('active');
    if ($('leaveFromMembersBtn')) $('leaveFromMembersBtn').onclick = () => {
    ...
    // 설정 모달 이벤트
    if ($('closeSettingsModal')) $('closeSettingsModal').onclick = closeSettingsModal;
    ...
    // 도움말 모달 이벤트
    if ($('helpBtn')) $('helpBtn').onclick = function () { $('helpModal').classList.add('active'); };
    if ($('closeHelpModal')) $('closeHelpModal').onclick = function () { $('helpModal').classList.remove('active'); };
    if ($('closeHelpBtn')) $('closeHelpBtn').onclick = function () { $('helpModal').classList.remove('active'); };
...
function handleMessageContextMenu(e) {
    ...
    // 2. 본인 메시지: 삭제
    if (isSent) {
        menuHtml += `<div class="context-menu-item danger" data-action="delete">🗑 삭제</div>`;
    }
    ...
    // 리액션 버튼
    const reactionBtn = menu.querySelector('[data-action="reaction"]');
    if (reactionBtn) {
        reactionBtn.onclick = () => {
            menu.remove();
            // [v4.1] 리액션 피커 팝업 표시
...
function updateMessageReactions(messageId, reactions) {
    ...
    reactionsContainer.innerHTML = reactions.map(r => {
        var isMine = r.user_ids && r.user_ids.includes(currentUser.id);
...
function renderReactionPicker(messageId) {
    return `<div class="reaction-picker">
        ${quickReactions.map(e => `<button class="reaction-picker-btn" onclick="toggleReaction(${messageId}, '${e}')">${e}</button>`).join('')}
    </div>`;
}
...
async function loadPinnedMessages() {
    if (!currentRoom) return;
    try {
        const pins = await api(`/api/rooms/${currentRoom.id}/pins`);
        currentPins = pins;
        updatePinnedBanner();
    } catch (e) {
...
async function pinCurrentMessage(messageId, content) {
    if (!currentRoom) return;
    try {
        const result = await api(`/api/rooms/${currentRoom.id}/pins`, {
            method: 'POST',
...
async function createPoll() {
    ...
    var optionInputs = $('pollOptions').querySelectorAll('input');
    var options = Array.from(optionInputs).map(i => i.value.trim()).filter(v => v);
    var multipleChoice = $('pollMultiple').checked;
    var anonymous = $('pollAnonymous').checked;
    ...
    try {
        const result = await api(`/api/rooms/${currentRoom.id}/polls`, {
            method: 'POST',
            body: JSON.stringify({ question, options, multiple_choice: multipleChoice, anonymous })
        });
        if (result.success) {
...
function updatePollDisplay(poll) {
    ...
    var totalVotes = poll.options.reduce((sum, o) => sum + o.vote_count, 0);
...
async function loadRoomPolls() {
    if (!currentRoom) return;
    try {
        const polls = await api(`/api/rooms/${currentRoom.id}/polls`);
        // 투표 UI 업데이트 로직
    } catch (e) {
...
function renderFileList(files) {
    ...
    container.innerHTML = files.map(file => {
        ...
        var icon = isImage ? `<img src="/uploads/${file.file_path}" alt="">` : '📄';
        return `<div class="file-item">
            <div class="file-item-icon">${icon}</div>
            <div class="file-item-info">
                <div class="file-item-name">${escapeHtml(file.file_name)}</div>
                <div class="file-item-meta">${file.uploader_name} · ${formatDate(file.uploaded_at)}</div>
            </div>
            <div class="file-item-actions">
                <a href="/uploads/${file.file_path}" download class="icon-btn" title="다운로드">⬇️</a>
            </div>
        </div>`;
    }).join('');
}
...
async function checkAdminStatus() {
    if (!currentRoom) return;
    try {
        const result = await api(`/api/rooms/${currentRoom.id}/admin-check`);
...
async function loadAdminMemberList() {
    try {
        const roomInfo = await api(`/api/rooms/${currentRoom.id}/info`);
        const admins = await api(`/api/rooms/${currentRoom.id}/admins`);
        var adminIds = admins.map(a => a.id);

        var container = $('adminMemberList');
        if (!container) return;

        container.innerHTML = roomInfo.members.map(m => {
            ...
            return `<div class="user-item" style="display:flex; justify-content:space-between; align-items:center; padding:12px 0;">
                ...
                <button class="btn btn-sm ${isAdmin ? 'btn-danger' : 'btn-secondary'}" 
                        onclick="toggleAdmin(${m.id}, ${!isAdmin})">${isAdmin ? '해제' : '지정'}</button>
            </div>`;
        }).join('');
    } catch (e) {
        console.error('Load admin list error:', e);
    }
}

async function toggleAdmin(userId, isAdmin) {
    try {
        const result = await api(`/api/rooms/${currentRoom.id}/admins`, {
            method: 'POST',
            body: JSON.stringify({ user_id: userId, is_admin: isAdmin })
        });
        if (result.success) {
            showToast(isAdmin ? '관리자로 지정되었습니다.' : '관리자 권한이 해제되었습니다.', 'success');
            loadAdminMemberList();
            socket.emit('admin_updated', { room_id: currentRoom.id, user_id: userId, is_admin: isAdmin });
        }
    } catch (e) {
...
function saveDraft() {
    ...
    if (content) {
        drafts[currentRoom.id] = content;
    } else {
        delete drafts[currentRoom.id];
    }
...
function loadDraft() {
    ...
    var drafts = JSON.parse(localStorage.getItem(draftKey) || '{}');
    if (drafts[currentRoom.id]) {
        input.value = drafts[currentRoom.id];
    }
}
...
function loadNotificationSettings() {
    var saved = localStorage.getItem('notification_settings');
    if (saved) {
...
function updateNotificationSettingsUI() {
    var modeRadios = document.querySelectorAll('input[name="notifyOption"]');
    modeRadios.forEach(r => r.checked = r.value === notificationSettings.mode);

    var soundCheckbox = $('notifySound');
    if (soundCheckbox) soundCheckbox.checked = notificationSettings.sound;
}

function saveNotificationSettings() {
    var mode = document.querySelector('input[name="notifyOption"]:checked');
    notificationSettings.mode = mode ? mode.value : 'all';
    notificationSettings.sound = $('notifySound').checked;
...
async function doAdvancedSearch() {
    var query = $('advSearchQuery').value.trim();
    var dateFrom = $('advSearchDateFrom').value;
    var dateTo = $('advSearchDateTo').value;
    var fileOnly = $('advSearchFileOnly').checked;

    try {
        const results = await api('/api/search/advanced', {
            method: 'POST',
            body: JSON.stringify({
                query: query || null,
...
function renderAdvancedSearchResults(results) {
    ...
    if (!results || results.length === 0) {
        container.innerHTML = '<div style="text-align:center; padding:20px; color:var(--text-muted);">검색 결과가 없습니다</div>';
        return;
    }

    container.innerHTML = results.map(r => `
        <div class="search-result-item" onclick="goToMessage(${r.room_id}, ${r.id})">
            <div class="search-result-room">${escapeHtml(r.room_name || '')}</div>
            <div class="search-result-content">${escapeHtml(r.content || '[파일]')}</div>
            <div class="search-result-meta">${r.sender_name} · ${formatDate(r.created_at)}</div>
        </div>
    `).join('');
}

function goToMessage(roomId, messageId) {
    $('advancedSearchModal').classList.remove('active');
    // 해당 방으로 이동하고 메시지 하이라이트 (구현 필요)
}
...
document.addEventListener('DOMContentLoaded', function () {
    // 투표 모달
    var closePollModal = $('closePollModal');
    if (closePollModal) closePollModal.onclick = () => $('pollModal').classList.remove('active');

    var addPollOption = $('addPollOption');
    if (addPollOption) addPollOption.onclick = function () {
        var container = $('pollOptions');
    ...
    var createPollBtn = $('createPollBtn');
    if (createPollBtn) createPollBtn.onclick = createPoll;

    var createPollBtn2 = $('createPollBtn2');
    if (createPollBtn2) createPollBtn2.onclick = openPollModal;

    // 파일 저장소
    var closeFilesModal = $('closeFilesModal');
    if (closeFilesModal) closeFilesModal.onclick = () => $('filesModal').classList.remove('active');

    var viewFilesBtn = $('viewFilesBtn');
    if (viewFilesBtn) viewFilesBtn.onclick = openFilesModal;
    ...
    // 관리자 설정
    var closeAdminModal = $('closeAdminModal');
    if (closeAdminModal) closeAdminModal.onclick = () => $('adminModal').classList.remove('active');

    var adminSettingsBtn = $('adminSettingsBtn');
    if (adminSettingsBtn) adminSettingsBtn.onclick = openAdminModal;

    // 알림 설정
    var closeNotificationSettingsModal = $('closeNotificationSettingsModal');
    if (closeNotificationSettingsModal) {
        closeNotificationSettingsModal.onclick = () => $('notificationSettingsModal').classList.remove('active');
    }

    var saveNotificationSettingsBtn = $('saveNotificationSettings');
    if (saveNotificationSettingsBtn) saveNotificationSettingsBtn.onclick = saveNotificationSettings;

    // 고급 검색
    var closeAdvancedSearchModal = $('closeAdvancedSearchModal');
    if (closeAdvancedSearchModal) {
        closeAdvancedSearchModal.onclick = () => $('advancedSearchModal').classList.remove('active');
    }

    var doAdvancedSearchBtn = $('doAdvancedSearch');
    if (doAdvancedSearchBtn) doAdvancedSearchBtn.onclick = doAdvancedSearch;

    // 공지 배너 닫기
    var closePinnedBanner = $('closePinnedBanner');
    if (closePinnedBanner) closePinnedBanner.onclick = (e) => {
        e.stopPropagation();
    ...
    if (messageInput) {
        messageInput.addEventListener('input', debounce(saveDraft, 1000));
    }
...
// 방 선택 시 호출할 초기화 함수
var originalSelectRoom = typeof selectRoom === 'function' ? selectRoom : null;
...
// 메시지 전송 후 드래프트 클리어
var originalSendMessage = typeof sendMessage === 'function' ? sendMessage : null;
    ...
    if (changePwBtn) {
        changePwBtn.onclick = async () => {
            ...
            try {
                ...
                showToast(e.message || '비밀번호 변경 실패', 'error');
            }
        };
    }
    ...
    if (deleteAccBtn) {
        deleteAccBtn.onclick = async () => {
            ...
            try {
                ...
                showToast(e.message || '탈퇴 실패', 'error');
            }
        };
    }
});
