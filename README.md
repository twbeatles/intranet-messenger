# 🔒 사내 웹메신저 v3.7

Flask + Socket.IO + PyQt6 기반의 **종단간 암호화(E2E)** 사내 웹 메신저 시스템입니다.

## ✨ v3.7 업데이트 (2024-12)

### 🖥️ PyQt6 GUI 개선
- 📺 **HiDPI 지원**: 고해상도 디스플레이 자동 스케일링
- 🔔 **토스트 알림**: 서버 시작/중지 시 페이드 애니메이션 알림
- ⚡ **타입 힌트 추가**: 코드 품질 및 IDE 지원 향상

### 🐛 버그 수정
- sender_name null 체크 추가 (TypeError 방지)
- 데이터베이스 예외 처리 강화 (로깅 개선)

### 🧹 코드 정리
- 레거시 `messenger_server.py` (2320줄) 제거
- 모듈화된 `app/` 패키지 구조로 완전 전환
- 버전 번호 통합

---

## ✨ v3.6 업데이트 (2024-12)

### 🎨 UI/UX 개선
- 🔔 **토스트 알림 개선**: 스택형 알림, 진행률 바, 마우스 일시정지
- ✓✓ **읽음 표시 강화**: 더블 체크 아이콘 스타일 추가
- 🔢 **스크롤 배지**: 새 메시지 개수 표시
- 🎬 **모달 애니메이션**: 부드러운 열기/닫기 효과
- 💬 **빈 상태 개선**: 플로팅 애니메이션, 액션 버튼
- 💀 **스켈레톤 UI**: 로딩 중 스켈레톤 표시

### ⌨️ 키보드 단축키
| 단축키 | 기능 |
|--------|------|
| `Ctrl+K` | 대화 검색 |
| `Ctrl+N` | 새 대화 시작 |
| `Ctrl+F` | 대화 내 검색 |
| `ESC` | 모달/검색 닫기 |

### 🔧 안정성 개선
- 🧹 **메모리 누수 방지**: setInterval 관리 시스템
- 📡 **오프라인 지원**: 오프라인 배너, 자동 재연결
- ♿ **접근성 개선**: 포커스 표시, ARIA 레이블, 모션감소 모드

### 🐛 버그 수정
- nickname null check 5개 수정 (TypeError 방지)
- 경로 트래버설 보안 취약점 수정
- bare except 예외 처리 개선

---

## ✨ v3.5 업데이트

- 🎨 **랜덤 프로필 색상**: 유저마다 고유한 아바타 색상 (12가지 팔레트)
- 🔄 **새로고침 로그인 유지**: /api/me 세션 체크로 자동 로그인 복원
- ↩️ **답장 하이라이트 개선**: 새 메시지에도 즉시 하이라이트 적용
- ✏️ **메시지 수정/삭제**: 자신의 메시지 수정 및 삭제 기능
- 📍 **읽지 않은 메시지 표시**: "여기서부터 읽지 않음" 구분선
- ⬇️ **스크롤 하단 버튼**: 채팅창 하단으로 빠르게 이동

## ✨ v3.4 업데이트

- 🎨 **테마 커스터마이징**: 10가지 색상 프리셋 (Emerald, Ocean, Purple 등)
- 🌗 **라이트/다크 모드**: 시스템 설정 연동 + 수동 선택
- 🖼 **채팅 배경**: 도트, 그리드, 그라데이션 패턴
- 👤 **프로필 관리**: 닉네임 변경, 프로필 사진, 상태 메시지

---

## 🔒 사내망/폐쇄망 호환성

이 애플리케이션은 **외부 인터넷 연결 없이** 완전히 독립적으로 동작합니다:

- ✅ Socket.IO 라이브러리 로컬 포함 (`/static/js/socket.io.min.js`)
- ✅ CryptoJS 라이브러리 로컬 포함 (`/static/js/crypto-js.min.js`)
- ✅ 시스템 폰트만 사용 (외부 웹폰트 없음)
- ✅ 외부 CDN, API 호출 없음
- ✅ SQLite 로컬 데이터베이스

## 🌐 브라우저 호환성

| 브라우저 | 최소 버전 | 권장 버전 |
|---------|----------|----------|
| Chrome | 55+ | 최신 |
| Edge | 79+ | 최신 |
| Firefox | 52+ | 최신 |
| Safari | 11+ | 최신 |

> ⚠️ **Internet Explorer는 지원하지 않습니다.**

---

## 주요 기능

### 보안
- **종단간 암호화 (E2E)** - AES-256으로 서버 관리자도 메시지 확인 불가
- **HTTPS** - SSL/TLS 암호화 통신
- **경로 트래버설 방지** - 파일 업로드 보안 강화
- **비밀번호 솔트 해싱**

### 채팅
- **1:1 / 그룹 대화**
- **읽음 확인** (더블 체크 표시)
- **대화 내 검색** (Ctrl+F)
- **파일/이미지 첨부** (드래그앤드롭)
- **이모지**
- **@멘션**
- **답장**

### 알림
- **데스크톱 알림** - 새 메시지 수신 시 브라우저 알림
- **토스트 알림** - 스택형, 진행률 표시
- **오프라인 캐싱** - 서버 재시작 시에도 최근 메시지 유지

### 서버 관리 (PyQt6 GUI)
- **시스템 트레이**
- **HTTPS 활성화/비활성화**
- **SSL 인증서 생성**
- **실시간 통계**

---

## 📦 설치

### 기본 설치
```powershell
pip install flask flask-socketio simple-websocket pycryptodome pyqt6 cryptography
```

### 고성능 동시 접속 지원 (권장)
```powershell
pip install flask flask-socketio pycryptodome pyqt6 cryptography gevent gevent-websocket
```

---

## 🚀 실행

### GUI 모드 (권장)
```powershell
python server.py
```

### CLI 모드
```powershell
python server.py --cli
```

---

## 🗂️ 파일 구조

```
사내 메신저/
├── server.py                # 메인 진입점
├── config.py                # 설정 파일
├── messenger.spec           # PyInstaller 패키징 스펙
├── app/
│   ├── __init__.py          # Flask 앱 팩토리
│   ├── routes.py            # HTTP 라우트
│   ├── sockets.py           # Socket.IO 이벤트
│   ├── models.py            # 데이터베이스
│   └── utils.py             # 유틸리티
├── gui/
│   └── server_window.py     # PyQt6 GUI
├── certs/
│   ├── generate_cert.py     # 인증서 생성
│   ├── cert.pem             # SSL 인증서
│   └── key.pem              # 개인키
├── static/
│   ├── css/style.css        # 스타일시트 (~2900줄)
│   └── js/
│       ├── app.js           # 메인 앱 로직 (~2700줄)
│       ├── notification.js  # 알림 모듈
│       ├── storage.js       # 로컬 저장소
│       ├── socket.io.min.js # Socket.IO (로컬)
│       └── crypto-js.min.js # CryptoJS (로컬)
├── templates/
│   └── index.html           # HTML 템플릿
├── uploads/                 # 파일 업로드
└── messenger.db             # SQLite DB
```

---

## 📦 PyInstaller 패키징

### spec 파일 사용 (권장)
```powershell
cd "d:\google antigravity\사내 메신저"
pyinstaller messenger.spec
```

### auto-py-to-exe 사용
1. `auto-py-to-exe` 실행
2. Script: `server.py` 선택
3. One Directory 선택
4. Window Based 선택
5. Additional Files에 폴더 추가: `static/`, `templates/`, `app/`, `gui/`, `certs/`, `config.py`

### 결과물
```
dist/사내메신저v3.6/
├── 사내메신저v3.6.exe
├── static/
├── templates/
├── app/
├── gui/
├── certs/
└── config.py
```

---

## ⚙️ 설정 파일 (config.py)

```python
ASYNC_MODE = 'gevent'       # 'gevent' (권장), 'threading'
PING_TIMEOUT = 120          # 연결 타임아웃 (초)
PING_INTERVAL = 25          # 핑 간격 (초)
USE_HTTPS = False           # HTTPS 활성화
DEFAULT_PORT = 5000         # 서버 포트
```

---

## 📊 성능 비교

| 비동기 모드 | 동시 접속 | 설치 필요 |
|------------|----------|----------|
| `gevent` (권장) | 수백~수천 명 | `pip install gevent gevent-websocket` |
| `threading` | 5~10명 | 기본 포함 |

---

## ⚠️ 주의사항

- 첫 실행 시 `messenger.db`, `uploads/`, `server.log` 자동 생성
- HTTPS 사용 시 SSL 인증서 자동 생성 (cryptography 필요)
- **사내망에서 외부 인터넷 없이 완전히 독립 동작**
- **gevent 설치 권장**: 수십 명 이상 동시 접속 시 필수
