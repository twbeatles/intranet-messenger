...
- 타입 기준선
  - 현재 작업 환경에서는 Flask/PyQt/PyCryptodome 계열 import 미설치로 `pyright app gui` import resolution 실패 가능
- 테스트 기준선
...
- Pin 생성 room-bound 검증
  - `POST /api/rooms/<room_id>/pins`는 `message_id`가 현재 room 소속인지 확인한다
...
- 사용자 대상 소켓 이벤트
  - `room_list_updated`: affected user의 room list reload 트리거
  - `room_access_revoked`: 강퇴/나가기/탈퇴 시 현재 room 접근 철회 통지
- `user_profile_updated` payload는 `status_message`를 포함한다
- `send_message` 클라이언트 허용 타입은 `text|file|image`만 허용한다 (`system` 금지)
- `reply_to`, pin `message_id`, `message_read.message_id`는 모두 room-bound 검증을 거친다
- 파일 저장소 삭제 시 linked attachment message는 `message_deleted` 소켓 계약으로 같이 정리된다
